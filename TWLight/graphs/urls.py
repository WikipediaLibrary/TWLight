...
csv_urlpatterns = [
    url(
        r"^app_time_histogram/$",
        views.CSVAppTimeHistogram.as_view(),
        name="app_time_histogram",
    ),
    url(r"^app_medians/$", views.CSVAppMedians.as_view(), name="app_medians"),
    url(
        r"^app_distribution/$",
    ...
    url(
        r"^page_views/(?P<path>[a-zA-Z0-9_\-/]+)/$",
        views.CSVPageViewsByPath.as_view(),
        name="page_views_by_path",
    ),
    url(
        r"^proxy_authorizations_renewals/$",
    ...
    url(r"^user_language/$", views.CSVUserLanguage.as_view(), name="user_language"),
]
