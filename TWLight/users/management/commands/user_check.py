    ...
    def handle(self, **options):
        ...
        for user in users:
            try:
                ...
                self.stdout.write(u'{username}: no editor object.'.format(username=(user.username)))
            ...
            try:
                ...
                self.stdout.write(u'{name}: ID mismatch - local ID: {twlight_sub} remote ID: {sul_id}'.format(name=user.editor.wp_username,twlight_sub=user.editor.wp_sub,sul_id=global_userinfo['id']))
                pass
            except:
                pass
